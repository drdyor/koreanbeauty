 ...
 */
export function useSleepSummaries(
  userId: string,
  startDate: string,
  endDate: string
) {
  return useQuery({
    queryKey: ['sleep-summaries', userId, startDate, endDate],
    queryFn: () => healthService.getSleepSummaries(userId, startDate, endDate),
...
export function useSleepSessions(
  ...
  return useQuery({
    queryKey: ['sleep-sessions', userId, startDate, endDate],
    queryFn: () => healthService.getSleepSessions(userId, startDate, endDate),
    enabled: !!userId && !!startDate && !!endDate,
  });
}
