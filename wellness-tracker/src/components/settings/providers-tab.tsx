...
export function ProvidersTab() {
  ...
  useEffect(() => {
    if (providers && providers.length > 0 && !hasInitialized) {
      ...
      providers.forEach((provider) => {
        initial[provider.provider || provider.id] = provider.is_enabled ?? false;
      });
  ...
  const hasChanges = useMemo(() => {
    ...
    return providers.some(
      (provider) => localToggleStates[provider.provider || provider.id] !== (provider.is_enabled ?? false)
    );
        ...
        <div className="divide-y divide-zinc-800/50">
          {providers.map((provider) => (
            <ProviderItem
              ...
              provider={provider}
              localToggleState={
                localToggleStates[provider.provider || provider.id] ?? (provider.is_enabled ?? false)
              }
              onToggle={() => handleToggleProvider(provider.provider || provider.id)}
            />
          ))}
        </div>
      </div>
